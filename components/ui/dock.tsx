...
function DockItem({ children, className }: DockItemProps) {
  ...
  // Reduce the middle value (magnification) to decrease size on hover
  ...
  const hoverSound = useMemo(() => {
    if (typeof window !== "undefined") {
      return new Audio("/water-droplet.mp3");
    }
  }, []);
  ...
  return (
    <motion.div
      ...
      onHoverStart={() => {
        isHovered.set(1);
        if (hoverSound) hoverSound.play(); // Play the sound when hover starts
      }}
      onHoverEnd={() => {
        isHovered.set(0);
        if (hoverSound) {
      ...
      onFocus={() => {
        isHovered.set(1);
        if (hoverSound) hoverSound.play(); // Play sound on focus
      }}
      onBlur={() => {
        isHovered.set(0);
        if (hoverSound) {
      ...
      onClick={() => {
        if (clickSound) clickSound.play(); // Play the click sound on click
      }}
      className={cn(
